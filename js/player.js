...
Depois esta página vai receber dados assim:
...
function preencherInformacoesVideo(video = {}) {
    // Se houver categoria, mostrar a tag; caso contrário, mostrar um padrão
    if (videoCategoria) {
        videoCategoria.textContent = video.categoria || "Categorizado";
    }
    if (videoTitulo) videoTitulo.textContent = video.titulo || "Vídeo indisponível";
    if (videoDescricao) videoDescricao.textContent = video.descricao || "As informações deste vídeo aparecerão aqui quando os dados forem carregados.";
    if (videoDuracao) videoDuracao.textContent = formatarTextoOuPadrao(video.duracao);
    if (videoPublicacao) videoPublicacao.textContent = formatarTextoOuPadrao(video.dataPublicacao);
    if (videoViews) videoViews.textContent = formatarTextoOuPadrao(video.views);

    if (analyticsViews) analyticsViews.textContent = formatarTextoOuPadrao(video.views);
    if (analyticsLikes) analyticsLikes.textContent = formatarTextoOuPadrao(video.likes);
    if (analyticsFavoritos) analyticsFavoritos.textContent = formatarTextoOuPadrao(video.favoritos);
    if (analyticsRetencao) analyticsRetencao.textContent = formatarTextoOuPadrao(video.retencao);
...
function prepararEntradaVideo(video = {}) {
    ...
    // Se houver trailer/intro, utiliza primeiro em loop controlado.
...
function configurarAcoesRecomendacao(card, video) {
    ...
    card.addEventListener("keydown", (event) => {
        if (event.key === "Enter" || event.key === " ") {
            const focadoEhBotao = document.activeElement && document.activeElement.tagName === "BUTTON";
...
function criarCardRecomendacao(video) {
    ...
    card.setAttribute("aria-label", `Abrir vídeo ${video.titulo}`);
...
/*
FUNÇÃO PREPARADA PARA USO FUTURO COM FIREBASE

Quando a integração estiver pronta, aqui será o lugar
para buscar o vídeo pelo ID vindo da URL.
*/
async function carregarVideoAtual() {
    ...
    if (window.BrasflixFirebase && typeof window.BrasflixFirebase.buscarVideoPorId === "function") {
        video = await window.BrasflixFirebase.buscarVideoPorId(videoId);
        comentarios = await window.BrasflixFirebase.buscarComentariosPorVideo(videoId);
    }

    if (!video) {
        // fallback local ou mensagem de erro
        video = {
            id: videoId,
            titulo: "Vídeo carregado",
            descricao: `O vídeo com ID "${videoId}" será carregado aqui quando a integração com Firebase estiver pronta.`,
            categoria: "",
            duracao: "",
            dataPublicacao: "",
            views: "",
    ...
    if (video?.categoria && window.BrasflixFirebase && typeof window.BrasflixFirebase.buscarRecomendacoesPorCategoria === "function") {
        recomendacoes = await window.BrasflixFirebase.buscarRecomendacoesPorCategoria(video.categoria);
    }

    // Adicione introUrl de vídeo no Firestore, se não existir, com fallback no mesmo objeto
    if (!video.introUrl) {
        video.introUrl = video.introUrl || "";
    }
...
function salvarMetricasEmocionais(metricas) {
    ...
    if (metricas.emocaoPredominante) {
        localStorage.setItem("brasflixEmotionDominante", metricas.emocaoPredominante);
    }
}
...
async function buscarVideosPorCategoriaFirebase(categoria) {
    ...
    // Suporte ao catálogo Angular/Firestore a partir do Firebase existente
    const catalogo = window.BrasflixFirebase?.buscarVideosPorCategoria
        || window.BrasflixVideoCatalog?.buscarVideosPorCategoria
        || null;
    ...
    try {
        const videos = await catalogo(categoria);
        if (Array.isArray(videos) && videos.length > 0) {
            return videos;
        }
    } catch (error) {
        console.warn("Erro ao buscar videos do Firebase por categoria:", error);
    }
...
async function sugerirConteudosPorEmocao(emocao) {
    ...
    const recomendacoes =
        recomendacoesFirebase.length > 0
            ...
            : [
                  { id: "1", titulo: "Sugestão padrão 1", categoria, thumbnail: "imagens/placeholder.png" },
                  { id: "2", titulo: "Sugestão padrão 2", categoria, thumbnail: "imagens/placeholder.png" }
              ];
...
function iniciarMonitoramentoEmocional() {
    ...
    window.BrasflixEmotionAI.iniciarEmotionAcompanhamento({
        ...
        montarFrame: async (emocao, frame) => {
            // Pode adicionar overlay ou debug no futuro.
            return;
        },
        onUpdate: (data) => {
            ...
            if (window.PedriaCore && window.PedriaCore.config?.usarIA) {
                // opcional: enviar sinal de comportamento para IA.
