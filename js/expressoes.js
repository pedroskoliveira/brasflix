(function () {
    ...
    async function carregarModelosExpressoes() {
        ...
        try {
            await carregarEm(MODELS_PATH);
            if (status) status.textContent = "Modelos carregados de ./models.";
        } catch (erroModelosLocais) {
            console.warn("Falha ao carregar modelos locais (./models), tentando CDN:", erroModelosLocais);
