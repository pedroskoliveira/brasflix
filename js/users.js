...
import { collection, getDocs, doc, getDoc, updateDoc, arrayUnion, arrayRemove } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
function getAvatar(user = {}) { return user.avatar || user.fotoURL || "imagens/logo.png"; }
function categories(user = {}) { return Array.isArray(user.categoriasFavoritas) ? user.categoriasFavoritas : []; }
...
async function toggleFollow(targetUid) {
  if (!auth.currentUser) return alert("Faça login para seguir pessoas.");
  const myRef = doc(db, "usuarios", auth.currentUser.uid);
  ...
  await updateDoc(targetRef, { seguidores: following ? arrayRemove(auth.currentUser.uid) : arrayUnion(auth.currentUser.uid) });
...
function openChat(user) {
  if (window.BrasflixUserChat?.openConversationWith) {
    ...
    alert("O chat de usuários ainda não carregou.");
  }
}

function card(user) {
  const tags = categories(user).slice(0,3).map((tag) => `<span class="user-card-tag">${tag}</span>`).join("") || `<span class="user-card-tag">Sem categorias</span>`;
  ...
  wrapper.querySelector('.btn-chat')?.addEventListener('click', () => openChat({ uid: user.uid || user.id, nome: user.nome || '', email: user.email || '', fotoURL: getAvatar(user) }));
...
async function loadUsers() {
  await carregarPerfilAtual();
  const [pubSnap, userSnap] = await Promise.all([getDocs(collection(db, 'usuarios_publicos')), getDocs(collection(db, 'usuarios'))]);
  ...
  pubSnap.forEach((d) => {
    ...
    if (auth.currentUser?.uid && merged.uid === auth.currentUser.uid) return;
