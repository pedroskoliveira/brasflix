...
const AIEngine = {
  ...
  async coletarContextoUsuario(uid) {
    if (!uid) return { historico: [], comentarios: [], favoritos: [] };
    try {
      const [historicoSnap, comentariosSnap, usuariosSnap] = await Promise.all([
        getDocs(query(collection(db, "historico_visualizacoes"), where("userId", "==", uid), orderBy("timestamp", "desc"), limit(10))),
        getDocs(query(collection(db, "comentarios"), where("userId", "==", uid), orderBy("timestamp", "desc"), limit(10))),
        getDocs(query(collection(db, "usuarios"), where("usuarioId", "==", uid), limit(1)))
      ]);
  ...
  montarContextoTexto(contexto = {}) {
    ...
    if (Array.isArray(contexto.comentarios) && contexto.comentarios.length) {
      partes.push("Comentários recentes:\n" + contexto.comentarios.map((item, i) => `${i + 1}. ${item.texto || ""}`).join("\n"));
    }
    return partes.join("\n\n");
  },

  async gerar({ prompt, system = "Você é a PedrIA da BRASFLIX. Responda em português do Brasil, de forma útil e objetiva.", provider = null, incluirContextoUsuario = true }) {
    try {
      ...
      if (incluirContextoUsuario && uid) {
        ...
        if (contextoTexto) context.push({ role: "system", content: `Contexto do usuário:\n${contextoTexto}` });
      }

      const prov = provider || this.providerPadrao;
      let resposta = prov === "mistral"
        ? await mistralClient.sendMessage({ message: prompt, context, systemInstruction: system })
        : await geminiClient.sendMessage({ message: prompt, context, systemInstruction: system });

      if (!resposta?.ok && prov !== "mistral") {
        resposta = await mistralClient.sendMessage({ message: prompt, context, systemInstruction: system });
      }

      if (!resposta?.ok) {
        return { sucesso: false, origem: resposta?.provider || prov, erro: resposta?.error || "Falha ao gerar resposta." };
      }

      return { sucesso: true, origem: resposta.provider || prov, texto: resposta.text || "", aviso: resposta.notice || "" };
    } catch (error) {
      console.error("[AIEngine] Erro:", error);
      return { sucesso: false, origem: provider || this.providerPadrao, erro: error.message || "Erro interno na IA." };
    }
  }
};
